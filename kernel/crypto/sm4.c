...
{
    return T0[(uint8_t)(X >> 24)] ^ T1[(uint8_t)(X >> 16)] ^
           T2[(uint8_t)(X >> 8)] ^ T3[(uint8_t)X];
}
...
void SM4_KeySchedule(uint8_t MK[16], uint32_t rk[32])
{
    uint32_t K[36];
    int i;
    K[0] = ((uint32_t)MK[0] << 24) | ((uint32_t)MK[1] << 16) | ((uint32_t)MK[2] << 8) | (uint32_t)MK[3];
    K[1] = ((uint32_t)MK[4] << 24) | ((uint32_t)MK[5] << 16) | ((uint32_t)MK[6] << 8) | (uint32_t)MK[7];
    K[2] = ((uint32_t)MK[8] << 24) | ((uint32_t)MK[9] << 16) | ((uint32_t)MK[10] << 8) | (uint32_t)MK[11];
    K[3] = ((uint32_t)MK[12] << 24) | ((uint32_t)MK[13] << 16) | ((uint32_t)MK[14] << 8) | (uint32_t)MK[15];
    K[0] ^= FK[0];
    K[1] ^= FK[1];
    K[2] ^= FK[2];
    K[3] ^= FK[3];
...
static void SM4_Encrypt(uint8_t plaintext[16], uint8_t ciphertext[16], uint32_t rk[32])
{
    uint32_t X[36];
    int i;
    X[0] = ((uint32_t)plaintext[0] << 24) | ((uint32_t)plaintext[1] << 16) | ((uint32_t)plaintext[2] << 8) | (uint32_t)plaintext[3];
    X[1] = ((uint32_t)plaintext[4] << 24) | ((uint32_t)plaintext[5] << 16) | ((uint32_t)plaintext[6] << 8) | (uint32_t)plaintext[7];
    X[2] = ((uint32_t)plaintext[8] << 24) | ((uint32_t)plaintext[9] << 16) | ((uint32_t)plaintext[10] << 8) | (uint32_t)plaintext[11];
    X[3] = ((uint32_t)plaintext[12] << 24) | ((uint32_t)plaintext[13] << 16) | ((uint32_t)plaintext[14] << 8) | (uint32_t)plaintext[15];
    for (i = 0; i < 32; i++)
    {
        uint32_t tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i];
        X[i + 4] = X[i] ^ T(tmp);
    }
    ciphertext[0] = (X[35] >> 24) & 0xFF;
    ciphertext[1] = (X[35] >> 16) & 0xFF;
    ciphertext[2] = (X[35] >> 8) & 0xFF;
    ciphertext[3] = X[35] & 0xFF;
    ciphertext[4] = (X[34] >> 24) & 0xFF;
    ciphertext[5] = (X[34] >> 16) & 0xFF;
    ciphertext[6] = (X[34] >> 8) & 0xFF;
    ciphertext[7] = X[34] & 0xFF;
    ciphertext[8] = (X[33] >> 24) & 0xFF;
    ciphertext[9] = (X[33] >> 16) & 0xFF;
    ciphertext[10] = (X[33] >> 8) & 0xFF;
    ciphertext[11] = X[33] & 0xFF;
    ciphertext[12] = (X[32] >> 24) & 0xFF;
    ciphertext[13] = (X[32] >> 16) & 0xFF;
    ciphertext[14] = (X[32] >> 8) & 0xFF;
    ciphertext[15] = X[32] & 0xFF;
}

static void SM4_Decrypt(uint8_t ciphertext[16], uint8_t plaintext[16], uint32_t rk[32])
{
    uint32_t X[36];
    int i;
    X[0] = ((uint32_t)ciphertext[0] << 24) | ((uint32_t)ciphertext[1] << 16) | ((uint32_t)ciphertext[2] << 8) | (uint32_t)ciphertext[3];
    X[1] = ((uint32_t)ciphertext[4] << 24) | ((uint32_t)ciphertext[5] << 16) | ((uint32_t)ciphertext[6] << 8) | (uint32_t)ciphertext[7];
    X[2] = ((uint32_t)ciphertext[8] << 24) | ((uint32_t)ciphertext[9] << 16) | ((uint32_t)ciphertext[10] << 8) | (uint32_t)ciphertext[11];
    X[3] = ((uint32_t)ciphertext[12] << 24) | ((uint32_t)ciphertext[13] << 16) | ((uint32_t)ciphertext[14] << 8) | (uint32_t)ciphertext[15];
    for (i = 0; i < 32; i++)
    {
        uint32_t tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[31 - i];
        X[i + 4] = X[i] ^ T(tmp);
    }
    plaintext[0] = (X[35] >> 24) & 0xFF;
    plaintext[1] = (X[35] >> 16) & 0xFF;
    plaintext[2] = (X[35] >> 8) & 0xFF;
    plaintext[3] = X[35] & 0xFF;
    plaintext[4] = (X[34] >> 24) & 0xFF;
    plaintext[5] = (X[34] >> 16) & 0xFF;
    plaintext[6] = (X[34] >> 8) & 0xFF;
    plaintext[7] = X[34] & 0xFF;
    plaintext[8] = (X[33] >> 24) & 0xFF;
    plaintext[9] = (X[33] >> 16) & 0xFF;
    plaintext[10] = (X[33] >> 8) & 0xFF;
    plaintext[11] = X[33] & 0xFF;
    plaintext[12] = (X[32] >> 24) & 0xFF;
    plaintext[13] = (X[32] >> 16) & 0xFF;
    plaintext[14] = (X[32] >> 8) & 0xFF;
    plaintext[15] = X[32] & 0xFF;
}

static uint32_t PKCS7_Padding(uint8_t *buffer, uint32_t inputLen, uint32_t bufferSize) 
{
    if (bufferSize < (inputLen + 16))
        return 0;
    uint8_t pad = 16 - (inputLen % 16);
    memset(buffer + inputLen, pad, pad);
    return inputLen + pad;
}

static uint32_t PKCS7_Unpadding(uint8_t *buffer, uint32_t inputLen)
{
    if (inputLen == 0)
        return 0;
    uint8_t pad = buffer[inputLen - 1];
    if (pad > 16 || pad == 0)
        return 0;
    return inputLen - pad;
}
...
{
    if (!data || !outputLen || !rk)
        return -1;
    uint32_t paddedLen = ((dataLen + 15) / 16) * 16;
    if (bufferSize < paddedLen)
        return -2;
    paddedLen = PKCS7_Padding(data, dataLen, bufferSize);
    if (paddedLen == 0)
        return -3;
    for (uint32_t i = 0; i < paddedLen; i += 16)
        SM4_Encrypt(data + i, data + i, rk);
    *outputLen = paddedLen;
    return 0;
}
...
int SM4_ECB_Decrypt(uint8_t *data, uint32_t dataLen, uint32_t *outputLen, const uint32_t rk[32])
{
    ...
    {
        SM4_Decrypt(data + i, temp, rk);
        memcpy(data + i, temp, 16);
    }
    uint32_t unpadded_len = PKCS7_Unpadding(data, dataLen);
    if (unpadded_len == 0)
        return -2;
    *outputLen = unpadded_len;
    return 0;
}