...
#define BIT_ALT_STAT_RDY        0x40
#define BIT_ALT_STAT_DRQ        0x08

#define BIT_DEV_MBS             0xa0
#define BIT_DEV_LBA             0x40
#define BIT_DEV_DEV             0x30

#define CMD_IDENTIFY            0xec
#define CMD_READ_SECTOR         0x20
#define CMD_WRITE_SECTOR        0x30

#define max_lba                 ((80*1024*1024/512)-1)  //可调，应该可以根据磁盘大小而变化，此处尚未实现
    ...
    while(part_idx++<4){
        if(p->fs_type==0x5){
            ...
            if(ext_lba==0){
                ...
                //ASSERT(!list_empty(&partition_list));
                sprintf(hd->prim_parts[p_no].name,"%s%d",hd->name,l_no+1);
                l_no++;
                if(l_no>=8){
                    return;
                }
...
void ide_init(){
    ...
    printk("\n all partition info\n");
    ASSERT(!list_traversal(&partition_list,partition_info,(int)NULL));
...
void ide_write(struct disk* hd,uint32_t lba,void* buf,uint32_t sec_cnt){
    ...
    while(secs_done<sec_cnt){
        ...
        select_sector(hd,lba+secs_done,secs_done);
